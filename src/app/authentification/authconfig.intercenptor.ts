      ...
      private handle403Error(request: HttpRequest<any>, next: HttpHandler) {
        if (!this.isRefreshing) {
          ...
          const token = localStorage.getItem('refresh_token');
            ...
            */
           /*
L'Angular SwitchMap mappe chaque valeur de l'observable source vers un observable interne, 
s'y abonne, puis commence à en émettre les valeurs. 
Il crée un nouvel observable intérieur pour chaque valeur qu'il reçoit de la Source. 
Chaque fois qu'il crée un nouvel observable interne, 
il se désabonne de tous les observables internes précédemment créés. 
Fondamentalement, il passe au plus récent observable en éliminant tous les autres.
           */
          if (token)
